...
- Normal Mode 에서 **q**를 입력하면 하단 상태표시줄에 q가 표시됨. 이는 앞으로 기록할 레지스터를 지정해주기를 대기하고 있는 상태
- 레지스터(a-z, 0-9 중 하나)를 정해 입력하여 매크로를 저장할 키 선정
- 매크로로 등록할 내가 원하는 동작을 시행
- **q**를 입력해 매크로 등록을 종료
- **@** 에 매크로를 저장한 키를 입력하여 매크로 실행
- a키에 매크로를 저장했다고 가정한 경우, ***@a : 매크로 1회 실행, @@ : 방금 실행한 매크로 실행, 10@a : 매크로 10회 실행***
